...
public class JabRefFrame extends JPanel implements OutputPrinter {
	...
	JLabel statusLine = new JLabel("", SwingConstants.LEFT), statusLabel = new JLabel(Globals.lang("Status") + ":",
			SwingConstants.LEFT);
	...
	public JToggleButton groupToggle, searchToggle, previewToggle, highlightAny, highlightAll;
	...
	AbstractAction close = new CloseDatabaseAction(),
			...
			help = new HelpAction("JabRef help", helpDiag, GUIGlobals.baseFrameHelp, Globals.lang("JabRef help"),
					prefs.getKey("Help")),
			contents = new HelpAction("Help contents", helpDiag, GUIGlobals.helpContents, Globals.lang("Help contents"),
					GUIGlobals.getIconUrl("helpContents")),
			about = new HelpAction("About JabRef", helpDiag, GUIGlobals.aboutPage, Globals.lang("About JabRef"),
					GUIGlobals.getIconUrl("about")),
			editEntry = new GeneralAction("edit", "Edit entry", Globals.lang("Edit entry"), prefs.getKey("Edit entry")),
			focusTable = new GeneralAction("focusTable", "Focus entry table",
					Globals.lang("Move the keyboard focus to the entry table"), prefs.getKey("Focus entry table")),
			save = new GeneralAction("save", "Save database", Globals.lang("Save database"), prefs.getKey("Save database")),
			saveAs = new GeneralAction("saveAs", "Save database as ...", Globals.lang("Save database as ..."),
					prefs.getKey("Save database as ...")),
			saveAll = new SaveAllAction(JabRefFrame.this),
			saveSelectedAs = new GeneralAction("saveSelectedAs", "Save selected as ...", Globals.lang("Save selected as ..."),
			...
			undo = new GeneralAction("undo", "Undo", Globals.lang("Undo"), prefs.getKey("Undo")),
			redo = new GeneralAction("redo", "Redo", Globals.lang("Redo"), prefs.getKey("Redo")),
			forward = new GeneralAction("forward", "Forward", Globals.lang("Forward"), "right", prefs.getKey("Forward")),
			back = new GeneralAction("back", "Back", Globals.lang("Back"), "left", prefs.getKey("Back")),
			/*
			 * cut = new GeneralAction("cut", "Cut", Globals.lang("Cut"),
			 * GUIGlobals.cutIconFile, prefs.getKey("Cut")),
			 */
			delete = new GeneralAction("delete", "Delete", Globals.lang("Delete"), prefs.getKey("Delete")),
			/*
			 * copy = new GeneralAction("copy", "Copy", Globals.lang("Copy"),
			 * GUIGlobals.copyIconFile, prefs.getKey("Copy")),
			 */
			...
			mark = new GeneralAction("markEntries", "Mark entries", Globals.lang("Mark entries"), prefs.getKey("Mark entries")),
			unmark = new GeneralAction("unmarkEntries", "Unmark entries", Globals.lang("Unmark entries"),
			...
			incrementalSearch = new GeneralAction("incSearch", "Incremental search", Globals.lang("Start incremental search"),
					prefs.getKey("Incremental search")),
			normalSearch = new GeneralAction("search", "Search", Globals.lang("Search"), prefs.getKey("Search")),
			...
			copyKey = new GeneralAction("copyKey", "Copy BibTeX key", prefs.getKey("Copy BibTeX key")),
			...
			// prefs.getKey("Open")),
			/*
			 * remove = new GeneralAction("remove", "Remove",
			 * "Remove selected entries", GUIGlobals.removeIconFile),
			 */
			selectAll = new GeneralAction("selectAll", "Select all", prefs.getKey("Select all")),
			replaceAll = new GeneralAction("replaceAll", "Replace string", prefs.getKey("Replace string")),

			editPreamble = new GeneralAction("editPreamble", "Edit preamble", Globals.lang("Edit preamble"),
					prefs.getKey("Edit preamble")),
			editStrings = new GeneralAction("editStrings", "Edit strings", Globals.lang("Edit strings"),
					prefs.getKey("Edit strings")),
			toggleGroups = new GeneralAction("toggleGroups", "Toggle groups interface", Globals.lang("Toggle groups interface"),
					prefs.getKey("Toggle groups interface")),
			togglePreview = new GeneralAction("togglePreview", "Toggle entry preview", Globals.lang("Toggle entry preview"),
					prefs.getKey("Toggle entry preview")),
			toggleHighlightAny = new GeneralAction("toggleHighlightGroupsMatchingAny",
					"Highlight groups matching any selected entry", Globals.lang("Highlight groups matching any selected entry"),
			...
			toggleHighlightAll = new GeneralAction("toggleHighlightGroupsMatchingAll",
					...
					Globals.lang("Highlight groups matching all selected entries"), GUIGlobals.getIconUrl("groupsHighlightAll")),
			switchPreview = new GeneralAction("switchPreview", "Switch preview layout", prefs.getKey("Switch preview layout")),
			makeKeyAction = new GeneralAction("makeKey", "Autogenerate BibTeX keys", Globals.lang("Autogenerate BibTeX keys"),
					prefs.getKey("Autogenerate BibTeX keys")),

			writeXmpAction = new GeneralAction("writeXMP", "Write XMP-metadata to PDFs",
					Globals.lang("Will write XMP-metadata to the PDFs linked from selected entries."), prefs.getKey("Write XMP")),
			openFile = new GeneralAction("openExternalFile", "Open file", Globals.lang("Open file"), prefs.getKey("Open file")),
			openPdf = new GeneralAction("openFile", "Open PDF or PS", Globals.lang("Open PDF or PS"),
					prefs.getKey("Open PDF or PS")),
			openUrl = new GeneralAction("openUrl", "Open URL or DOI", Globals.lang("Open URL or DOI"),
					prefs.getKey("Open URL or DOI")),
			openSpires = new GeneralAction("openSpires", "Open SPIRES entry", Globals.lang("Open SPIRES entry"),
					prefs.getKey("Open SPIRES entry")),
			dupliCheck = new GeneralAction("dupliCheck", "Find duplicates"),
			// strictDupliCheck = new GeneralAction("strictDupliCheck",
			// "Find and remove exact duplicates"),
			plainTextImport = new GeneralAction("plainTextImport", "New entry from plain text",
			...
			autoSetPdf = new GeneralAction("autoSetPdf", Globals.lang("Synchronize %0 links", "PDF"),
					Globals.prefs.getKey("Synchronize PDF")), autoSetPs = new GeneralAction("autoSetPs", Globals.lang(
					"Synchronize %0 links", "PS"), Globals.prefs.getKey("Synchronize PS")), autoSetFile = new GeneralAction(
					"autoSetFile", Globals.lang("Synchronize file links"), Globals.prefs.getKey("Synchronize files")),
			...
			unabbreviate = new GeneralAction("unabbreviate", "Unabbreviate journal names",
					Globals.lang("Unabbreviate journal names of the selected entries"), Globals.prefs.getKey("Unabbreviate")),
			manageJournals = new ManageJournalsAction(this), databaseProperties = new DatabasePropertiesAction(),
			upgradeExternalLinks = new GeneralAction("upgradeLinks", "Upgrade external links", Globals.lang(
					"Upgrade external PDF/PS links to use the '%0' field.", GUIGlobals.FILE_FIELD)),
			errorConsole = Globals.errorConsole.getAction(frame), test = new GeneralAction("test", "Test"),

			dbConnect = new GeneralAction("dbConnect", "Connect to external SQL database",
					Globals.lang("Connect to external SQL database"), GUIGlobals.getIconUrl("dbConnect")),

			dbExport = new GeneralAction("dbExport", "Export to external SQL database",
					Globals.lang("Export to external SQL database"), GUIGlobals.getIconUrl("dbExport")),
			dbImport = new DbImportAction(this).getAction(),
			// downloadFullText = new GeneralAction("downloadFullText",
			// "Look up full text document",
			// Globals.lang("Follow DOI or URL link and try to locate PDF full text document")),
			increaseFontSize = new IncreaseTableFontSizeAction(), decreseFontSize = new DecreaseTableFontSizeAction(),
			installPlugin = new PluginInstallerAction(this), resolveDuplicateKeys = new GeneralAction("resolveDuplicateKeys",
					"Resolve duplicate BibTeX keys", Globals.lang("Find and remove duplicate BibTeX keys"),
	...
	JMenu importMenu = subMenu("Import into current database"), importNewMenu = subMenu("Import into new database"),
			exportMenu = subMenu("Export"), customExportMenu = subMenu("Custom export"),
	...
	NewEntryAction[] newSpecificEntryAction = new NewEntryAction[] { new NewEntryAction("article", prefs.getKey("New article")),
			new NewEntryAction("book", prefs.getKey("New book")), new NewEntryAction("phdthesis", prefs.getKey("New phdthesis")),
			new NewEntryAction("inbook", prefs.getKey("New inbook")),
			new NewEntryAction("mastersthesis", prefs.getKey("New mastersthesis")),
			new NewEntryAction("proceedings", prefs.getKey("New proceedings")), new NewEntryAction("inproceedings"),
			new NewEntryAction("conference"), new NewEntryAction("incollection"), new NewEntryAction("booklet"),
			new NewEntryAction("manual"), new NewEntryAction("techreport"),
			new NewEntryAction("unpublished", prefs.getKey("New unpublished")), new NewEntryAction("misc"),
	...
	private void init() {
		...
		if (!prefs.getBoolean("windowMaximised")) {
			...
			//
			// Do not put a window outside the screen if the preference values
			// are wrong.
			//
			// Useful reference:
			// http://www.exampledepot.com/egs/java.awt/screen_ScreenSize.html?l=rel
			// googled on forums.java.sun.com graphicsenvironment second screen
			// java
			//
	...
	// General info dialog. The OSXAdapter calls this method when
	// "About OSXAdapter"
	// is selected from the application menu.
	public void about() {
		JDialog about = new JDialog(frame, Globals.lang("About JabRef"), true);
		JEditorPane jp = new JEditorPane();
		JScrollPane sp = new JScrollPane(jp, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
		...
		try {
			...
			ex.printStackTrace();
			JOptionPane.showMessageDialog(JabRefFrame.this, "Could not load file 'About.html'", "Error",
...
//					}
			...
			for (int i = 0; i < tabbedPane.getTabCount(); i++) {
				if (baseAt(i).baseChanged) {
					...
					int answer = JOptionPane.showConfirmDialog(JabRefFrame.this, Globals.lang("Database has changed. Do you "
							+ "want to save before closing?"), Globals.lang("Save before closing"),
		...
		if (close) {
			...
			for (int i = 0; i < tabbedPane.getTabCount(); i++) {
				if (baseAt(i).isSaving()) {
					// There is a database still being saved, so we need to
					// wait.
					WaitForSaveOperation w = new WaitForSaveOperation(this);
					w.show(); // This method won't return until cancelled or the
			...
			// prefs.putBoolean("windowMaximised",
	...
	private void macOSXRegistration() {
		if (Globals.osName.equals(Globals.MAC)) {
			try {
				...
				Class<?>[] defArgs = { JabRefFrame.class };
				...
				// boolean args,
				// use "boolean.class", then pass a Boolean object in as the
				// arg, which apparently
	...
	private void initLayout() {
		...
		// this.add(lim);
		/*
		 * JPanel empt = new JPanel(); empt.setBackground(GUIGlobals.lightGray);
		 * gbl.setConstraints(empt, con); this.add(empt);
		 * 
		 * con.insets = new Insets(1,0,1,1); con.anchor =
		 * GridBagConstraints.EAST; con.weightx = 0;
		 * gbl.setConstraints(searchManager, con); this).add(searchManager);
		 */
		con.gridwidth = GridBagConstraints.REMAINDER;
		con.weightx = 1;
		con.weighty = 0;
	...
	private void initLabelMaker() {
		...
		labelMaker.addRule(new ArticleLabelRule(), BibtexEntryType.ARTICLE);
		labelMaker.addRule(new BookLabelRule(), BibtexEntryType.BOOK);
		labelMaker.addRule(new IncollectionLabelRule(), BibtexEntryType.INCOLLECTION);
		labelMaker.addRule(new InproceedingsLabelRule(), BibtexEntryType.INPROCEEDINGS);
	}
	...
	class GeneralAction extends MnemonicAwareAction {
		...
		/*
		 * public GeneralAction(String command, String text, String description,
		 * URL imageUrl, KeyStroke key) { this.command = command; ImageIcon icon
		 * = GUIGlobals.getImage(command); if (icon != null)
		 * putValue(SMALL_ICON, icon); putValue(NAME, text);
		 * putValue(SHORT_DESCRIPTION, Globals.lang(description));
		 * putValue(ACCELERATOR_KEY, key); }
		 */
	...
	class NewEntryAction extends MnemonicAwareAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (tabbedPane.getTabCount() > 0) {
				((BasePanel) (tabbedPane.getSelectedComponent())).newEntry(BibtexEntryType.getType(thisType));
			}
			else {
	...
	/*
	 * private void setupDatabaseLayout() { // This method is called whenever
	 * this frame has been provided // with a database, and completes the
	 * layout.
	 * 
	 * 
	 * if (file != null) setTitle(GUIGlobals.baseTitle+file.getName()); else
	 * setTitle(GUIGlobals.untitledTitle);
	 * 
	 * //DragNDropManager dndm = new DragNDropManager(this);
	 * 
	 * //setNonEmptyState(); Util.pr("JabRefFrame: Must set non-empty state.");
	 * }
	 */
	...
	private void fillMenu() {
		...
		/*
		 * options.add(new AbstractAction("Font") { public void
		 * actionPerformed(ActionEvent e) { // JDialog dl = new
		 * EntryCustomizationDialog(JabRefFrame.this); Font f=new
		 * FontSelectorDialog (JabRefFrame.this,
		 * GUIGlobals.CURRENTFONT).getSelectedFont(); if(f==null) return; else
		 * GUIGlobals.CURRENTFONT=f; // updatefont prefs.put("fontFamily",
		 * GUIGlobals.CURRENTFONT.getFamily()); prefs.putInt("fontStyle",
		 * GUIGlobals.CURRENTFONT.getStyle()); prefs.putInt("fontSize",
		 * GUIGlobals.CURRENTFONT.getSize()); if (tabbedPane.getTabCount() > 0)
		 * { for (int i=0; i<tabbedPane.getTabCount(); i++) {
		 * baseAt(i).entryTable.updateFont(); baseAt(i).refreshTable(); } } }
		 * });
		 */
...
//		}
	...
	private void createToolBar() {
		...
		// tlb.add(new
		// JabRefLabel(GUIGlobals.frameTitle+" "+GUIGlobals.version));
	...
	protected void initActions() {
		...
		openDatabaseOnlyActions.addAll(Arrays.asList(new Object[] { manageSelectors, mergeDatabaseAction, newSubDatabaseAction,
				close, save, saveAs, saveSelectedAs, undo, redo, cut, delete, copy, paste, mark, unmark, unmarkAll, editEntry,
				importCiteSeer, selectAll, copyKey, copyCiteKey, editPreamble, editStrings, toggleGroups, toggleSearch,
				makeKeyAction, normalSearch, incrementalSearch, replaceAll, importMenu, exportMenu, fetchCiteSeer, openPdf,
				openUrl, openFile, openSpires, togglePreview, dupliCheck, /*
																		 * strictDupliCheck
																		 * ,
																		 */highlightAll, highlightAny, newEntryAction,
				plainTextImport, massSetField, closeDatabaseAction, switchPreview, integrityCheckAction, autoSetPdf, autoSetPs,
				toggleHighlightAny, toggleHighlightAll, databaseProperties, abbreviateIso, abbreviateMedline, unabbreviate,
				exportAll, exportSelected, importCurrent, saveAll, dbConnect, dbExport, focusTable }));
		...
		severalDatabasesOnlyActions.addAll(Arrays.asList(new Action[] { nextTab, prevTab, sortTabs }));
	 ...
	 * 
	 * @param list
	 *            List that should contain Actions and Components.
	 * @param enabled
	 */
	...
	public void addTab(BasePanel bp, File file, boolean raisePanel) {
		tabbedPane.add((file != null ? file.getName() : Globals.lang(GUIGlobals.untitledTitle)), bp);
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (d.getAction()) {
				...
				JOptionPane.showMessageDialog(JabRefFrame.this, Globals.lang("Your new key bindings have been stored.") + "\n"
						+ Globals.lang("You must restart JabRef for the new key " + "bindings to work properly."),
						Globals.lang("Key bindings changed"), JOptionPane.INFORMATION_MESSAGE);
			}
		}
	}
	...
	class ImportCiteSeerAction extends MnemonicAwareAction {
		...
		public ImportCiteSeerAction() {
			...
			putValue(ACCELERATOR_KEY, prefs.getKey("Import Fields from CiteSeer")); // Key
																					// defined
																					// in
																					// MenuTitles!
		}
			...
			if (citeSeerFetcher.activateImportFetcher()) {
					...
					public void run() {
						...
						if (rowCount >= 1) {
							...
							JOptionPane.showMessageDialog(currentBp.frame(),
									...
									Globals.lang("CiteSeer Import Error"), JOptionPane.WARNING_MESSAGE);
						}
						if (clickedOn != null) {
				...
				JOptionPane.showMessageDialog(
						...
						Globals.lang("A CiteSeer import operation is currently in progress.") + "  "
								+ Globals.lang("Please wait until it has finished."), Globals.lang("CiteSeer Import Error"),
			...
			if (citeSeerFetcher.activateCitationFetcher()) {
				...
				(new Thread() {
					...
					Runnable updateComponent = new Runnable() {
						...
						/*
						 * TODO: This should probably be selectable on/off in
						 * the preferences window, but for now all Citation
						 * fetcher operations will sort by citation count.
						 */
				...
				JOptionPane.showMessageDialog(
						...
						Globals.lang("A CiteSeer fetch operation is currently in progress.") + "  "
								+ Globals.lang("Please wait until it has finished."), Globals.lang("CiteSeer Fetch Error"),
						JOptionPane.WARNING_MESSAGE);
			}
		}
	}
	...
	class NewSubDatabaseAction extends MnemonicAwareAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (dialog.okPressed()) {
				BasePanel bp = new BasePanel(JabRefFrame.this, dialog.getGenerateDB(), // database
						null, // file
						(HashMap<String, String>) null, Globals.prefs.get("defaultEncoding")); // meta
																								// data
	...
	class IntegrityCheckAction extends AbstractAction {
		public IntegrityCheckAction() {
			...
			// putValue( SHORT_DESCRIPTION, "integrity" ) ; //Globals.lang(
			// "integrity" ) ) ;
			// putValue(MNEMONIC_KEY, GUIGlobals.newKeyCode);
		}
			...
			SwingUtilities.invokeLater(new Runnable() {
				...
				public void run() {
					ImportInspectionDialog diag = new ImportInspectionDialog(JabRefFrame.this, panel,
							BibtexFields.DEFAULT_INSPECTION_FIELDS, Globals.lang("Import"), openInNew);
	 ...
	 * 
	 * @param filename
	 *            If non-null, a message is printed to the status line
	 *            describing how many entries were imported, and from which
	 *            file. If null, the message will not be printed.
	 * @param intoNew
	 *            Determines if the entries will be put in a new database or in
	 *            the current one.
	 */
	public int addBibEntries(List<BibtexEntry> bibentries, String filename, boolean intoNew) {
		if (bibentries == null || bibentries.size() == 0) {
			...
			JOptionPane.showMessageDialog(JabRefFrame.this, Globals.lang("No entries found. Please make sure you are "
					+ "using the correct import filter."), Globals.lang("Import failed"), JOptionPane.ERROR_MESSAGE);
			return 0;
		}
		...
		if (intoNew || (tabbedPane.getTabCount() == 0)) {
			...
			mainLoop: for (BibtexEntry entry : bibentries) {
				...
				if (checkForDuplicates) {
					loop: for (Iterator<String> i2 = database.getKeySet().iterator(); i2.hasNext();) {
						BibtexEntry existingEntry = database.getEntryById(i2.next());
						if (DuplicateCheck.isDuplicate(entry, existingEntry)) {
							DuplicateResolverDialog drd = new DuplicateResolverDialog(frame, existingEntry, entry,
	 ...
	 * before using the progress bar, to set a maximum value appropriate to the
	 * task at hand.
	 * 
	 * If not called on the event dispatch thread, this method uses
	 * SwingUtilities.invokeLater() to do the actual operation on the EDT.
			...
			if (tabbedPane.getTabCount() > 0) {
				for (int i = 0; i < tabbedPane.getTabCount(); i++) {
					if (tabbedPane.getTitleAt(i).equals(GUIGlobals.untitledTitle)) {
						tabbedPane.setSelectedIndex(i);
						int answer = JOptionPane.showConfirmDialog(JabRefFrame.this, Globals
								.lang("This untitled database must be saved first to be "
										+ "included in the saved session. Save now?"), Globals.lang("Save database"),
	...
	class ChangeTabAction extends MnemonicAwareAction {
		...
		public ChangeTabAction(boolean next) {
			...
			putValue(ACCELERATOR_KEY, (next ? prefs.getKey("Next tab") : prefs.getKey("Previous tab")));
		}
	...
	class IncreaseTableFontSizeAction extends MnemonicAwareAction {
		...
		public void actionPerformed(ActionEvent event) {
			...
			Font newFont = new Font(GUIGlobals.CURRENTFONT.getFamily(), GUIGlobals.CURRENTFONT.getStyle(), currentSize + 1);
	...
	/*
	 * private class ForegroundLabel extends JLabel { public
	 * ForegroundLabel(String s) { super(s); setFont(new Font("plain",
	 * Font.BOLD, 70)); setHorizontalAlignment(JLabel.CENTER); }
	 * 
	 * public void paint(Graphics g) { Graphics2D g2 = (Graphics2D)g;
	 * g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
	 * RenderingHints.VALUE_ANTIALIAS_ON); super.paint(g2); //To change body of
	...
	private class MyGlassPane extends JPanel {
		...
		public MyGlassPane() {
			 ...
			 * 
			 * setLayout(new BorderLayout()); add(infoLabel,
			 * BorderLayout.CENTER);
			 */
			super.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
		}

		// Override isOpaque() to prevent the glasspane from hiding the window
		// contents:
