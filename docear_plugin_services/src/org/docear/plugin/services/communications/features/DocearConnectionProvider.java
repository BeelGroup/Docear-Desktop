...
public class DocearConnectionProvider implements IDocearServiceFeature {
	...
	protected WebResource getServiceResource() {		
	...
	protected static String readResponseContent(InputStream is) throws IOException {
		int chr;
	...
	protected ClientResponse post(WebResource webResource, Object requestEntity) throws Exception {
		return post(webResource.getRequestBuilder(), requestEntity);
	}

	protected ClientResponse post(Builder builder, Object requestEntity) throws Exception {
		if (SwingUtilities.isEventDispatchThread()) {
			throw new Exception("Never call the webservice from the event dispatch thread.");
		}
		synchronized (proxyAuthentication) {
